    ...
    <div class="layout">
        <section class="panel">
            <div class="panel-header">
                <div class="panel-title"><%= editando != null ? "Editar cupo" : "Activar cliente" %></div>
                <div class="panel-sub"><%= editando != null ? "Solo se modifica el cupo; los datos personales viven en Usuarios." : "Crea usuarios cliente desde Usuarios y luego ajusta su cupo aqui." %></div>
            </div>
            <div class="panel-body">
                <% if (!mensaje.isEmpty()) { %><div class="msg msg-<%= tipoMsg %>"><%= mensaje %></div><% } %>
                <% if (editando != null) { %><div class="badge-edit">Editando #<%= editando.getId_clientes() %></div><% } %>

                <form action="Clientes.jsp<%= embedParam %>" method="post">
                    <% if (editando != null) { %>
                        <input type="hidden" name="accion" value="actualizar">
                        <input type="hidden" name="id" value="<%= editando.getId_clientes() %>">
                        <div class="form-group">
                            <label>Cliente</label>
                            <input type="text" readonly value="<%= editando.getNombreCompleto() %>">
                        </div>
                    <% } else { %>
                        ...
                        <div class="form-group">
                            ...
                            <select name="id_usuario" required>
                                <% if (disponibles.isEmpty()) { %>
                                    <option value="">No hay usuarios cliente pendientes</option>
                                <% } else { for (Clientes c : disponibles) { %>
                                    <option value="<%= c.getIdUsuario() %>"><%= c.getNombreCompleto() %> - <%= c.getIdentificacion() %></option>
                                <% } } %>
                            </select>
                        </div>
                    <% } %>
                    <div class="form-group">
                        <label>Cupo de credito (COP)</label>
                        <input type="number" name="Credito" min="0" step="1000" required value="<%= editando != null ? editando.getCupo_credito() : "200000" %>">
                    </div>
                    <div class="btn-row">
                        <button type="submit" class="btn-primary"><%= editando != null ? "Guardar cupo" : "Activar cliente" %></button>
                        <% if (editando != null) { %><a href="Clientes.jsp<%= embedParam %>" class="btn-cancelar">Cancelar</a><% } else { %><button type="reset" class="btn-secondary">Limpiar</button><% } %>
                    </div>
                </form>
            </div>
        </section>
        ...
        <section class="panel">
            <div class="panel-header">
                ...
                <div class="panel-sub"><%= lista.size() %> cliente<%= lista.size() != 1 ? "s" : "" %> activos</div>
            </div>
                                ...
                                <td>
                                    <div class="actions">
                                        <form action="Clientes.jsp<%= embedParam %>" method="post">
                                            ...
                                            <input type="hidden" name="id" value="<%= c.getId_clientes() %>">
                                            <button type="submit" class="btn-edit">Editar</button>
                                        </form>
                                        <form action="Clientes.jsp<%= embedParam %>" method="post" onsubmit="return confirm('Eliminar la ficha de <%= c.getNombreCompleto() %>?');">
