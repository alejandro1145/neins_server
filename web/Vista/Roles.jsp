...
<%@page import="Controlador.RolesDAO, Modelo.Roles, java.util.List"%>
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="Roles.jsp" class="nav-item active">
...
<%
    ...
    if ("insertar".equals(accion)) {
        Roles r = new Roles();
        r.setDescrpcion_roles(request.getParameter("descripcion"));
        if (dao.insertar(r)) {
        ...
        Roles r = new Roles();
        r.setId_roles(Integer.parseInt(request.getParameter("id")));
        r.setDescrpcion_roles(request.getParameter("descripcion"));
        if (dao.actualizar(r)) {
...
%>
    ...
    <div class="layout">
        ...
        <!-- ── FORMULARIO ── -->
        <div class="form-card">
            <h2><%= editId > 0 ? "✏️ Editar Rol" : "➕ Nuevo Rol" %></h2>

            <% if (editId > 0) { %>
                <div class="edit-badge">✏️ Modo edición — Rol #<%= editId %></div>
            <% } %>
            ...
            <form action="Roles.jsp" method="post">
                ...
                <div class="btn-row">
                    <button type="submit" class="btn-primary">
                        <%= editId > 0 ? "💾 Guardar Cambios" : "➕ Registrar Rol" %>
                    </button>
                    <% if (editId > 0) { %>
                        <a href="Roles.jsp" class="btn-secondary">✕ Cancelar</a>
                    <% } else { %>
            ...
            <% } else { %>
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><%= r.getId_roles() %></td>
                            <td><span class="rol-badge">🔑 <%= r.getDescrpcion_roles() %></span></td>
                            <td>
                                <div class="action-btns">
                                    <!-- Botón Editar -->
                                    <form action="Roles.jsp" method="post" style="display:inline;">
                                        <input type="hidden" name="accion"      value="editar">
                                        <input type="hidden" name="id"          value="<%= r.getId_roles() %>">
                                        <input type="hidden" name="descripcion" value="<%= r.getDescrpcion_roles() %>">
                                        <button type="submit" class="btn-edit">✏️ Editar</button>
                                    </form>

                                    <!-- Botón Eliminar con confirmación -->
                                    <form action="Roles.jsp" method="post" style="display:inline;"
                                          onsubmit="return confirm('¿Eliminar el rol \'<%= r.getDescrpcion_roles() %>\'?\nEsta acción no se puede deshacer.');">
