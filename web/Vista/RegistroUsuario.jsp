...
<%
    ...
    if ("actualizar".equals(accion) && errorConexion == null) {
        ...
        u.setFechaNacimiento(request.getParameter("fecha_nacimiento"));
        u.setClaveExpira(LocalDate.now().plusDays(DIAS_EXPIRACION_CLAVE).toString());
...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box} body{margin:0;min-height:100vh;background:radial-gradient(circle at top left,rgba(216,169,61,.10),transparent 34%),var(--bg);font-family:'DM Sans',sans-serif;color:var(--text)}
        ...
        .code-notice{display:none;padding:14px 16px;border-radius:10px;margin-bottom:18px;background:rgba(216,169,61,.14);border:1px solid var(--line);color:#ffe4a3}.code-notice strong{font-size:20px;letter-spacing:4px;color:var(--gold2)}
...
<div class="page-wrap">
    ...
    <div class="card">
        ...
        <div id="codeNotice" class="code-notice">Código de verificación: <strong id="generatedCode"></strong><br><small>Modo local: cópialo en el campo de abajo. Vence en 10 minutos.</small></div>
        ...
        <form action="RegistroUsuario.jsp" method="post">
            ...
            <div class="form-grid">
                ...
                <div class="form-group"><label>Fecha de nacimiento</label><input type="date" name="fecha_nacimiento" required max="<%= LocalDate.now().toString() %>" value="<%= (editando != null && editando.getFechaNacimiento() != null) ? editando.getFechaNacimiento() : "" %>"></div>
...
(function () {
    ...
    btn.addEventListener('click', function () {
        ...
        fetch('<%= request.getContextPath() %>/EmailVerificationServlet', {
            ...
            if (data.ok) {
                var notice = document.getElementById('codeNotice');
                var generated = document.getElementById('generatedCode');
                if (notice && generated) { generated.textContent = data.devCode || ''; notice.style.display = 'block'; }
